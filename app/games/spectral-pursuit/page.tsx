...
      <div className="container mx-auto max-w-6xl space-y-10 md:space-y-14">
        ...
        <section className="text-center space-y-3 md:space-y-4">
          <p className="pixel-text text-xs text-bright-purple uppercase tracking-wider">
            {t.games['spectral-pursuit'].tagline}
          </p>
          <h1 className="pixel-text text-4xl md:text-5xl lg:text-6xl text-bright neon-text">
            {t.games['spectral-pursuit'].name}
          </h1>
          <p className="pixel-text text-bright text-sm md:text-base max-w-3xl mx-auto leading-relaxed px-4">
            {t.games['spectral-pursuit'].intro}
          </p>
          <button
            onClick={() => setShowDifficultySelector(true)}
            className="inline-block px-4 py-2 border border-bright-purple/60 text-bright pixel-text text-xs rounded hover:bg-bright-purple/20 transition-colors"
          >
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-black/40 border border-bright-cyan/60 rounded-lg p-4 md:p-5 space-y-3 hover:bg-black/50 transition-colors">
            <h2 className="pixel-text text-xs md:text-sm text-bright-cyan uppercase">{t.games['spectral-pursuit'].controls.title}</h2>
            <ul className="pixel-text text-xs md:text-sm text-bright space-y-1.5 md:space-y-2">
              <li>{t.games['spectral-pursuit'].controls.move}</li>
              <li>{t.games['spectral-pursuit'].controls.stealth}</li>
              <li>{t.games['spectral-pursuit'].controls.decoy}</li>
              <li>{t.games['spectral-pursuit'].controls.pause}</li>
              <li>{t.games['spectral-pursuit'].controls.restart}</li>
            </ul>
          </div>
          ...
          <div className="bg-black/40 border border-bright-pink/60 rounded-lg p-4 md:p-5 space-y-3 hover:bg-black/50 transition-colors">
            <h2 className="pixel-text text-xs md:text-sm text-bright-pink uppercase">{t.games['spectral-pursuit'].objectives.title}</h2>
            <ul className="pixel-text text-xs md:text-sm text-bright space-y-1.5 md:space-y-2">
              <li>{t.games['spectral-pursuit'].objectives.item1}</li>
              <li>{t.games['spectral-pursuit'].objectives.item2}</li>
              <li>{t.games['spectral-pursuit'].objectives.item3}</li>
              <li>{t.games['spectral-pursuit'].objectives.item4}</li>
            </ul>
          </div>
          ...
          <div className="bg-black/40 border border-bright-yellow/60 rounded-lg p-4 md:p-5 space-y-3 hover:bg-black/50 transition-colors">
            <h2 className="pixel-text text-xs md:text-sm text-bright-yellow uppercase">{t.games['spectral-pursuit'].notes.title}</h2>
            <ul className="pixel-text text-xs md:text-sm text-bright space-y-1.5 md:space-y-2">
              <li>{t.games['spectral-pursuit'].notes.item1}</li>
              <li>{t.games['spectral-pursuit'].notes.item2}</li>
              <li>{t.games['spectral-pursuit'].notes.item3}</li>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur">
      <div className="w-full max-w-md bg-black/80 border border-bright-purple/60 rounded-xl shadow-neon-purple p-6 space-y-4">
        <div className="space-y-2 text-center">
          <p className="pixel-text text-xs text-bright-purple uppercase">{t.gameUI.submitScore}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <label className="block text-left pixel-text text-bright text-xs">
            ...
            <input
              ...
              className="mt-1 w-full rounded border border-bright-purple/50 bg-black/60 px-3 py-2 pixel-text text-bright focus:outline-none focus:ring-2 focus:ring-bright-purple"
              maxLength={18}
              required
            />
          </label>
