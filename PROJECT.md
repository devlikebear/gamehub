...
- [x] **업적 시스템 (뱃지/트로피)** [#28](https://github.com/devlikebear/gamehub/issues/28) ✅
  - [x] 업적 데이터 구조 및 타입 정의 (5단계 티어, 7개 카테고리)
  - [x] 업적 해금 로직 시스템 (LocalStorage 기반 진행도 추적)
  - [x] 업적 카드 UI 컴포넌트 (티어별 그라디언트 & 글로우 효과)
  - [x] 업적 알림 시스템 (해금 시 실시간 토스트 알림)
  - [x] 업적 페이지 (`/achievements`) - 필터링, 통계, 진행도 표시
  - [x] 게임 통합 유틸리티 (integration helper, React hook)
  - [x] Stellar Salvo 게임 통합 (킬 카운트, 시간, 에너지 추적)
  - [x] 8개 게임 기본 업적 정의 (각 게임별 1개 이상)
  - [x] i18n 지원 (한국어/영어)
  - [x] 텍스트 가시성 개선 (색상 최적화)
- [ ] 게임 업적 확장 (게임당 5-10개 업적 추가)
- [ ] 나머지 7개 게임에 업적 통합
- [ ] 게임 통계 (플레이 시간, 플레이 횟수)
- [ ] 키보드 단축키 가이드
- [ ] 난이도 선택 (쉬움/보통/어려움)
...
**마지막 업데이트**: 2025-10-08
**프로젝트 상태**: Phase 7 진행 중 (업적 시스템 완료, 게임 통합 진행)
